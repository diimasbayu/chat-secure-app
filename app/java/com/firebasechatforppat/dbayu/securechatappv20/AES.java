...
public class AES {
     ...
     * <p>
     * Contents are reset on each major call (setKey, encrypt, decrypt) and
     * should be used after each of these calls returns for display.
     */
    ...
    /**
     * static code to initialise the log and alog tables. Used to implement
    ...
    public static byte[] encrypt(byte[] plain) {
        ...
        if (traceLevel > 1) {
            traceInfo += "n  R" + numRounds + " (Key = " + Util.toHEX1(Ker) + ")t";
        }
    ...
    public static byte[] decrypt(byte[] cipher) {
        ...
        for (int r = 1; r < numRounds; r++) {
            ...
            if (traceLevel > 2) {
                traceInfo += "nt AK = " + Util.toHEX1(ta);
            }
